...
def SolveEquation(a, b, c, n, m,conditions,problem_type):
        # Kiểm tra nếu tồn tại b <= 0 thì kết luận vô nghiệm nhưng vẫn giải ra bằng đơn hình
    if any(num <= 0 and num != 0 for num in b):
        print("Bài toán không có giải pháp do tồn tại b <= 0.")
        print("Tuy nhiên, chúng ta sẽ tiếp tục giải bằng phương pháp đơn hình để xác định giải pháp tối ưu.")
    elif any(num == 0 for num in b):
        print("Bài toán với b = 0 được tiếp tục giải bằng phương pháp Bland") 
    ...
    while not all(num >= 0 for num in tableau[-1][:-1]):
        ...
        if pivot_element is None:
            ...
            break
        print("Phần tử chốt đã chọn:", pivot_element.row, ",", pivot_element.column," có giá trị là:",tableau[pivot_element.row][pivot_element.column])
        print("---------------------------------------------")
    ...
    if check_unbounded(tableau):
        ...
        for i in range(m):
            if conditions[i] == ">= 0" and ans[i] < 0:
                ...
                print(f"Giá trị x{i+1} không thõa điều kiện >= 0.")
